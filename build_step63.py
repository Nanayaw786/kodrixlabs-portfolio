...
whatsapp_snippet = """<a href="https://wa.me/""" + whatsapp_number + """?text=""" + prefilled_message.replace(" ", "%20") + """" class="whatsapp-float" target="_blank" rel="noopener" aria-label="Chat on WhatsApp">
...
for page in pages:
    ...
    if "whatsapp-float" in content:
        print(page + ": WhatsApp button already present (skipped)")
        continue

    if anchor in content:
        content = content.replace(anchor, whatsapp_snippet + anchor, 1)
        with open(page, "w") as file:
            file.write(content)
        print(page + ": WhatsApp button added successfully")
    else:
